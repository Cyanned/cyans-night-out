...
class DialogueBox extends FlxSpriteGroup {
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>) {
		...
		currentCutImage = new FlxSprite(0, 0).loadGraphic(Paths.image("Cutscenes/CUT 1"), false, 1, 1, false);
	...
	override function update(elapsed:Float) {
		...
		if (FlxG.keys.justPressed.ANY && dialogueStarted == true) {
			...
			if (dialogueList[2] == null && dialogueList[1] != null) {
				if (!isEnding) {
					isEnding = true;
				...
				dialogueList.remove(dialogueList[1]);
				startDialogue();
			}
		}
	...
	function startDialogue():Void {
		...
		swagDialogue.resetText(dialogueList[1]);
	...
	var cutSceneArray = [' CUT 0', 'CUT 1', 'CUT 2', 'CUT 3', 'CUT 4', 'CUT 5', 'CUT 6', 'CUT 7'];
	...
	{
		var splitName:Array<String> = dialogueList[1].split(":");
		curCharacter = splitName[0];
		dialogueList[1] = dialogueList[1].substr(splitName[1].length + 2).trim();

		var splitSong:Array<String> = dialogueList[0].split("|");
		curSong = splitSong[0];
		dialogueList[0] = dialogueList[0].substr(splitSong[0].length + 2).trim();
	}
}
